...
class ItemViewModel(private val itemId: String?, private val itemRepository: ItemRepository) :
    ...
    fun saveOrUpdateItem(text: String, date: String, review: Int, watched: Boolean, latitude: Double, longitude: Double) {
        viewModelScope.launch {

            Log.d(TAG, "saveOrUpdateItem...");
            try {
                uiState = uiState.copy(submitResult = Result.Loading)
                val item = uiState.item.copy(title = text, date = date, review = review, watched = watched, latitude = latitude, longitude = longitude)
                val savedItem: Item;
                if (itemId == null) {
