...
type PaginationFunction = (item: ItemProps) => Promise<ItemProps[]>;
type ItemsFn = (itemsAux: ItemProps[]) => void;
...
export const ItemProvider: React.FC<ItemProviderProps> = ({ children }) => {
    ...
    const setItems = useCallback<ItemsFn>(setItemsCallback, []);
    const value = { items, fetching, fetchingError, saving, savingError, saveItem, paginationFunction, setItems, reviews };
    const {saveServerPhotos} = usePhotos();
    /*useEffect(handleSaveServerSidePhotos, [items]);
    function handleSaveServerSidePhotos() {
        savePhotos();
        async function savePhotos() {
            const itemsAux: ItemProps[] = await getItems(token);
            const allPhotos: MyPhoto[] = [];
            if(items) {
                for (let i = 0; i < itemsAux.length; i++) {
                    allPhotos.push(itemsAux[i].photo!);
                }
                saveServerPhotos(allPhotos);
            }
        }
    }*/
    log('returns');
    ...
    async function setItemsCallback(itemsAux: ItemProps[]) {
        await setItems();
        function setItems()
        {
    ...
    async function paginationCallBack(item: ItemProps): Promise<ItemProps[]> {
        if(items)
        {
            const itemsFinal: ItemProps[] = await fetchPaginationData();
            return itemsFinal;
        }
        return [];
        async function fetchPaginationData()
        {
            try {
                ...
                for(let i = index; i < NUMBER_OF_MOVIES_PER_PAGE + index && i < itemsAux.length; i++)
                {
        ...
        async function fetchItemsConnected() {
            try {
                ...
                if (!canceled) {
                    ...
                    let itemsFinal:ItemProps[] = [];
                    for(let i = 0; i < Math.max(index, NUMBER_OF_MOVIES_PER_PAGE) && i<itemsAux.length; i++)
                    {
                        itemsFinal.push(itemsAux[i]);
                    }
