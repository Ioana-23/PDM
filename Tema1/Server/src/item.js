...
export class ItemStore {
  ...
  async insert(item) {
    ...
    if(!item.review) {
      throw new Error('Missing review property')
    }
    if(!item.date) {
...
itemRouter.get('/filter/:review', async (ctx) => {
  const userId = ctx.state.user._id;
  const item = await itemStore.find({ review: ctx.params.review, userId });
  if (item) {
    if (item.userId === userId) {
      ctx.response.body = item;
      ctx.response.status = 200; // ok
...
itemRouter.put('/:id', async ctx => {
  ...
  if (!itemId) {
    ...
    item.userId = userId;
    console.log({itemId})
    const updatedCount = await itemStore.update({ _id: itemId }, item);
    if (updatedCount === 1) {
      response.body = item;
